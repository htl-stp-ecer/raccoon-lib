...
1. **`libstp-map` C++-Modul.** `WorldMap` aus libstp-sim als Basis, `TableMap`-Queries aus Python portiert, räumlicher Index, Batch-API. Python-Bindings ersetzen `TableMap` 1:1. **Python-Datei wird gelöscht.** `RobotGeometry` mit raus. **`MapCorrectedOdometry` wird hart gedroppt** — keine Migration, keine Adapter, weg. Bestehende Tests laufen unverändert (oder werden gelöscht, wo sie nur die Korrekturschicht testen).
   _Status (Mai 2026): ✅ erledigt, Commits `4aef856` (libstp-map), `dc6e2ef` (libstp-sim alias), `527270a` (TableMap-Alias), `2104dbd` (MapCorrectedOdometry weg)._
...
4. **Motions bekommen `absolute_target`-Pfad — harter Umstieg.** Kein Adapter, kein Doppel-Pfad. Alle Motions werden auf das absolute Target umgezogen, `WorldCorrectionMiddleware` wird in derselben Phase ersatzlos entfernt. Drumbot-Toleranzen aus `374cd5b` werden hier zurückgedreht. Mehr Risiko in einer Phase, aber kein Code-Doppel mit Lifetime > 0.
   _Status (Mai 2026): ✅ erledigt. Motion-Klassen absolute-only, `odometry.reset()` raus aus `start()` (Commit `1a229d0`); PathExecutor füttert `target_heading_rad` pro Segment aus `localization.get_pose()` (`6d2c941`); Korrektur-Layer + WorldPoseTracker komplett gelöscht (`75694fc`); AbsolutePathExecutor obsolet, weg (`d0d5638`); Python-Heading-Reads gehen durch Localization (`b7c9bd4`); Drumbot-Toleranzen zurückgedreht._
5. **DSL desugart** auf absolute Plan-IR. Konservativer Optimizer (nur `validate_reachable` + `fold_implicit_turns`). Default flippt für neue Code-Pfade; alte DSL bleibt funktional.
   _Status (Mai 2026): Großteils erledigt. Relativ-zu-absolut-Desugaring ist als Python-Compile-Pfad ergänzt (`compile_relative_to_absolute`, `nodes_to_absolute`): intended Pose-Chain ab `(0,0,0)`, `Segment` → `Goto`/`TurnTo`, `SideAction` → `Action`. Erste konservative Passes existieren (`validate_reachable`, `fold_implicit_turns`) und sind auf dem Helper default-on. `smooth_path()` bevorzugt jetzt intern einen Absolute-IR-Compile-Pfad und brückt diesen konservativ zurück in die Legacy-`Segment`-IR, inklusive absoluter Heading-Targets für lineare Segmente und absolute `TurnTo`-Auflösung über Localization am Segmentstart. Unsupported Fälle fallen bewusst auf den Legacy-Pfad zurück statt still falsch zu kompilieren: `Defer`, konditionsbasierte Unknown-Endpoints, `Resync` und aktuell auch Arc-`Goto`-Bridging. Fokus-Tests decken Drive/Turn/Drive, Lateral, Arc, Action-Barrieren, Map-Bounds sowie den `smooth_path()`-Default-Flip/Fallback ab. Noch offen: echte absolute Runtime-Ausführung für world-frame `Goto`-Positionen (statt konservativem Bridge-back), Arc-/Resync-Ausführung auf dem absoluten Pfad und breitere Snapshot-/Sim-Regressionen für den Default-Flip._
6. **Particle Filter** ersetzt den Pass-Through in Localization. Resync-Steps (`align_to_wall`, `find_line`, `resync_at_start_pose`) werden funktional. Performance-Profil auf Pi3.
   ...
   _Status (Mai 2026, Phase-6 Inkrement): begonnen. `libstp-localization` nutzt jetzt einen deterministisch konfigurierbaren Particle-Filter-Kern statt des reinen Pass-Throughs; `Localization::observe(Observation)` ist als sigma-gewichtetes Pose-Update funktional und enthält bewusst eine konfigurierbare Observation-Injection, damit explizite Resync-Schritte den Filter auch nach größerem Drift wieder einklinken können. Noch offen: map-/sensorbasierte Likelihoods (`sensorIsOnLine`/`sensorIsOnWall`), echte `align_to_wall`/`find_line`-Stepper-End-to-End-Wiring und Pi3-Performance-Profiling._
