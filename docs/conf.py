...
import os
autoapi_type = 'python'
_stubs_dir = '../python/stubs'
_python_dir = '../python'
_module_python_dirs = sorted(
    path for path in glob.glob(os.path.join(os.path.dirname(__file__), '../modules/*/python'))
    if os.path.isdir(path)
)
_extra_python_dirs = [
    '../raccoon-transport/python',
]
_extra_python_dirs = [
    path for path in _extra_python_dirs
    if os.path.isdir(os.path.join(os.path.dirname(__file__), path))
]
# Use stubs if available (generated by pybind11-stubgen), otherwise fall back to source
if os.path.isdir(os.path.join(os.path.dirname(__file__), _stubs_dir)):
    autoapi_dirs = [_stubs_dir, _python_dir, *_module_python_dirs, *_extra_python_dirs]
    autoapi_ignore = []
else:
    autoapi_dirs = [_python_dir, *_module_python_dirs, *_extra_python_dirs]
    autoapi_ignore = []
