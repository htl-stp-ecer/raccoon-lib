...
libstp::motion::DifferentialDrive::DifferentialDrive(device::Device* device,
    const ahrs::AttitudeEstimator& attitude_estimator): attitudeEstimator(attitude_estimator),
                                                        vXPid(utility::PIDController()),
                                                        vYPid(utility::PIDController()),
                                                        wPid(utility::PIDController()),
                                                        headingPid(utility::PIDController()),
                                                        device(device), state(DifferentialDriveState([device]
                                                        {
...
{
    const auto hasRotation = std::fabs(absoluteSpeed.angularRad) > EPSILON;
    const auto shouldTargetHeading = std::fabs(state.desiredHeading) > EPSILON;
