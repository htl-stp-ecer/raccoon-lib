...
void libstp::sensors::GyroSensor::calibrate(std::shared_ptr<MatrixX3d> calibrationMatrix)
{
    ...
    SPDLOG_INFO("[IMU] Calibrated gyro sensors with bias: ({}, {}, {}), variance: ({}, {}, {})",
                (*offset)[0], (*offset)[1], (*offset)[2], 
                (*variance)[0], (*variance)[1], (*variance)[2]);
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::GyroSensor::getValue() const
{
...
std::shared_ptr<Eigen::Vector3d> libstp::sensors::GyroSensor::getVariance()
{
    return variance;
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::GyroSensor::getBias()
{
    return offset;
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::GyroSensor::applyCalibration(const std::shared_ptr<Vector3d>& sample) const
{
    return std::make_shared<Vector3d>(*sample - *offset);
}

void libstp::sensors::AccelSensor::calibrate(std::shared_ptr<MatrixX3d> calibrationMatrix)
{
    const int rows = calibrationMatrix->rows();
    const int cols = calibrationMatrix->cols();
    ...
    SPDLOG_INFO("[IMU] Calibrated accel sensors with bias: ({}, {}, {}), variance: ({}, {}, {})",
                (*offset)[0], (*offset)[1], (*offset)[2], 
                (*variance)[0], (*variance)[1], (*variance)[2]);
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::AccelSensor::getValue() const
{
    return applyCalibration(std::make_shared<Vector3d>(Vector3d(accel_x(), accel_y(), accel_z())));
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::AccelSensor::getVariance()
{
    return variance;
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::AccelSensor::getBias()
{
    return offset;
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::AccelSensor::getGravity()
{
    return gravity;
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::AccelSensor::applyCalibration(const std::shared_ptr<Vector3d>& sample) const
{
    return std::make_shared<Vector3d>(*sample - *offset);
}

void libstp::sensors::MagnetoSensor::calibrate(std::shared_ptr<MatrixX3d> calibrationMatrix)
{
    variance = std::make_shared<Vector3d>(calibrationMatrix->colwise().squaredNorm());
    SPDLOG_INFO("[IMU] Calibrated magneto sensors with variance: ({}, {}, {})",
                (*variance)[0], (*variance)[1], (*variance)[2]);
}

void libstp::sensors::MagnetoSensor::setHardIronOffset(const std::shared_ptr<Vector3d>& offset)
{
    this->offset = offset;
}

void libstp::sensors::MagnetoSensor::setSoftIronMatrix(const std::shared_ptr<Matrix3d>& matrix)
{
    softIronMatrix = matrix;
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::MagnetoSensor::getValue() const
{
    const auto raw = std::make_shared<Vector3d>(Vector3d(magneto_x(), magneto_y(), magneto_z()));
    return applyCalibration(raw);
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::MagnetoSensor::getVariance()
{
    return variance;
}

std::shared_ptr<Eigen::Vector3d> libstp::sensors::MagnetoSensor::applyCalibration(const std::shared_ptr<Vector3d>& sample) const
{
    const auto corrected = std::make_shared<Vector3d>(*sample - *offset);
    return std::make_shared<Vector3d>(*softIronMatrix * *corrected);
}

libstp::async::AsyncAlgorithm<int> libstp::sensors::IMU::calibrate(const int sampleCount)
{
...
std::tuple<std::shared_ptr<Eigen::Vector3d>, std::shared_ptr<Eigen::Vector3d>, std::shared_ptr<Eigen::Vector3d>> libstp::sensors::IMU::getReading() const
{
