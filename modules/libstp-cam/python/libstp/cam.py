...
Usage::
    ...
    from libstp.cam import CamSensor
...
for _fname in os.listdir(_pkg_dir):
    if _fname.startswith("cam.") and (_fname.endswith(".so") or _fname.endswith(".pyd")):
        ...
        _ext_spec = importlib.util.spec_from_file_location("libstp._cam_native", _ext_path)
        break
...
# Expose the native CamSensor directly so ``from libstp.cam import CamSensor`` works.
