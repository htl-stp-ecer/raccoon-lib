...
{
    void createOmniWheeledBindings(py::module_& m);
}

namespace libstp::device::two_wheeled
{
    void createTwoWheeledBindings(py::module_& m);
}

namespace libstp::device::omni_wheeled::datatype
{
    void createOmniWheeledDatatypeBindings(py::module_& m);
}

PYBIND11_MODULE(device, m) {
    m.doc() = "Python bindings for libstp-device";

    libstp::device::createDeviceBindings(m);
    libstp::device::omni_wheeled::createOmniWheeledBindings(m);
    libstp::device::omni_wheeled::datatype::createOmniWheeledDatatypeBindings(m);
    libstp::device::two_wheeled::createTwoWheeledBindings(m);
}
