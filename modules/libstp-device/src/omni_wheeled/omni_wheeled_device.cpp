...
#include "device/omni_wheeled/omni_wheeled_device.h"
...
{
    throw std::logic_error("getDrivenDistanceForward is not implemented for OmniWheeledDevice. "
                         "Use getDrivenDistanceStrafe instead.");
...
{
    throw std::logic_error("getDrivenDistanceStrafe is not implemented for OmniWheeledDevice. "
                         "Use getDrivenDistanceStrafe instead.");
    /*const double radPerTick = 2.0 * M_PI / ticksPerRevolution;
...
void libstp::device::omni_wheeled::OmniWheeledDevice::applyKinematicsModel(const datatype::AbsoluteSpeed& speed)
{
    ...
    frontRightMotor.setSpeed(static_cast<int>(std::round(wheelSpeeds[0] * radToRev * ticksPerRevolution)));
    frontLeftMotor.setSpeed(static_cast<int>(std::round(wheelSpeeds[1] * radToRev * ticksPerRevolution)));
    rearLeftMotor.setSpeed(static_cast<int>(std::round(wheelSpeeds[2] * radToRev * ticksPerRevolution)));
    rearRightMotor.setSpeed(static_cast<int>(std::round(wheelSpeeds[3] * radToRev * ticksPerRevolution)));
}

std::tuple<float, float, float> libstp::device::omni_wheeled::OmniWheeledDevice::getWheelVelocities(
    const float dtSeconds)
{
    throw std::logic_error("getWheelVelocities is not implemented for OmniWheeledDevice. "
                         "Use getWheelVelocitiesWithEncoders instead.");
...
{
    throw std::logic_error("computeDrivenDistance is not implemented for OmniWheeledDevice. "
                         "Use computeDrivenDistanceStrafe instead.");
    /*const double radPerTick = 2.0 * M_PI / ticksPerRevolution;
