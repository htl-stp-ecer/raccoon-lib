...
def test_forward_1s_moves_significantly(results):
    ...
    assert x > 70.0, f"forward_1s: robot barely moved — x={x:.2f}, expected > 70"
    assert x < 150.0, f"forward_1s: robot overshot — x={x:.2f}, expected < 150"
    assert abs(y - 50.0) < 6.0, f"forward_1s: unexpected lateral drift — y={y:.2f}"
    assert (
        abs(theta) < 0.25
...
def test_forward_until_cm_stops_near_target(results):
    ...
    assert 58.0 < x < 80.0, f"forward_until_cm: stopped at unexpected x={x:.2f}, expected ~70 ± 12"
    assert abs(y - 50.0) < 6.0, f"forward_until_cm: unexpected lateral drift — y={y:.2f}"
...
def test_spin_rotates_without_translating(results):
    ...
    assert abs(x - 50.0) < 10.0, f"spin_1s: unexpected x translation — x={x:.2f}"
    assert abs(y - 50.0) < 10.0, f"spin_1s: unexpected y translation — y={y:.2f}"
    total_rotation = abs(theta)
    assert total_rotation > 0.2, (
        f"spin_1s: rotation too small — theta={math.degrees(theta):.1f}°, " f"expected > 11°"
    )
...
def test_zero_velocity_no_drift(results):
    ...
    assert abs(x - 50.0) < 2.0, f"zero_velocity: unexpected x drift — x={x:.2f}"
    assert abs(y - 50.0) < 2.0, f"zero_velocity: unexpected y drift — y={y:.2f}"
    assert (
        abs(theta) < 0.1
