...
def _make_digital_sensor():
    return SimpleNamespace(port=0, read=lambda: True)


def _make_analog_sensor():
    return SimpleNamespace(port=0, read=lambda: 512)
...
class TestWallAlignValidation:
    ...
    def test_all_directions(self):
        ...
        for cls in (WallAlignForward, WallAlignBackward, WallAlignStrafeLeft, WallAlignStrafeRight):
            with pytest.raises(ValueError, match="speed"):
                cls(speed=-1)
...
class TestMotorStepValidation:
    ...
    def test_stop_motor_check(self):
        from raccoon.step.motor.steps import MotorBrake, MotorOff, MotorPassiveBrake
