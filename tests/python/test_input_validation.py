...
class TestOnWhiteValidation:
    def test_valid(self):
        from libstp.step.condition import on_white
...
class TestAfterSecondsValidation:
    def test_valid(self):
        from libstp.step.condition import after_seconds
        after_seconds(5.0)
...
class TestAfterCmValidation:
    def test_valid(self):
        from libstp.step.condition import after_cm
        after_cm(10.0)
...
class TestAfterDegreesValidation:
    def test_valid(self):
        from libstp.step.condition import after_degrees
        after_degrees(90)
...
class TestOnDigitalValidation:
    def test_valid(self):
        from libstp.step.condition import on_digital
...
class TestOnAnalogValidation:
    def test_above_valid(self):
        from libstp.step.condition import on_analog_above
    ...
    def test_below_valid(self):
        from libstp.step.condition import on_analog_below
...
class TestStallDetectedValidation:
    def test_valid(self):
        from libstp.step.condition import stall_detected
...
class TestCustomConditionValidation:
    def test_valid_lambda(self):
        from libstp.step.condition import custom
...
class TestConditionCompositionValidation:
    ...
    def test_chained_valid(self):
        from libstp.step.condition import after_seconds, after_cm
        # a > b > c
...
class TestDriveValidation:
    def test_valid_cm(self):
        from libstp.step.motion.drive import DriveForward
        DriveForward(cm=25)
    ...
    def test_all_drive_variants(self):
        ...
        from libstp.step.motion.drive import (
...
class TestTurnValidation:
    def test_valid_degrees(self):
        from libstp.step.motion.turn import TurnLeft
        TurnLeft(degrees=90)
    ...
    def test_speed_out_of_range(self):
        from libstp.step.motion.turn import TurnRight
        with pytest.raises(ValueError, match="speed"):
    ...
    def test_all_turn_variants(self):
        from libstp.step.motion.turn import TurnLeft, TurnRight
...
class TestWallAlignValidation:
    def test_valid(self):
        from libstp.step.motion.wall_align import WallAlignForward
        WallAlignForward()
    ...
    def test_speed_negative(self):
        from libstp.step.motion.wall_align import WallAlignBackward
        with pytest.raises(ValueError, match="speed"):
    ...
    def test_all_directions(self):
        from libstp.step.motion.wall_align import (
...
class TestMotorStepValidation:
    def test_valid(self):
        from libstp.step.motor.steps import SetMotorPower
    ...
    def test_velocity_motor_check(self):
        from libstp.step.motor.steps import SetMotorVelocity
        with pytest.raises(TypeError, match="Motor"):
            SetMotorVelocity("not a motor", 800)

    def test_dps_motor_check(self):
        from libstp.step.motor.steps import SetMotorDps
        with pytest.raises(TypeError, match="Motor"):
            SetMotorDps("not a motor", 180.0)

    def test_move_to_motor_check(self):
        from libstp.step.motor.steps import MoveMotorTo
        with pytest.raises(TypeError, match="Motor"):
            MoveMotorTo("bad", 500)
    ...
    def test_move_relative_motor_check(self):
        from libstp.step.motor.steps import MoveMotorRelative
        with pytest.raises(TypeError, match="Motor"):
    ...
    def test_stop_motor_check(self):
        from libstp.step.motor.steps import MotorOff, MotorBrake, MotorPassiveBrake
        for cls in (MotorOff, MotorBrake, MotorPassiveBrake):
            with pytest.raises(TypeError, match="Motor"):
                cls("not a motor")

    def test_all_motor_steps_accept_valid_motor(self):
        from libstp.step.motor.steps import (
...
class TestServoStepValidation:
    def test_valid_set_position(self):
        from libstp.step.servo.steps import SetServoPosition
    ...
    def test_slow_servo_valid(self):
        from libstp.step.servo.steps import SlowServo
    ...
    def test_shake_servo_valid(self):
        from libstp.step.servo.steps import ShakeServo
...
class TestDeferValidation:
    def test_valid(self):
        from libstp.step.logic.defer import Defer
...
class TestRunValidation:
    def test_valid(self):
        from libstp.step.logic.defer import Run
...
class TestWaitForCheckpointValidation:
    def test_valid(self):
        from libstp.step.timing.wait_for_checkpoint import WaitForCheckpoint
...
class TestDoUntilCheckpointValidation:
    def test_valid(self):
        from libstp.step.timing.do_until_checkpoint import DoUntilCheckpoint
...
class TestCompositeValidation:
    def test_sequential_rejects_non_steps(self):
        from libstp.step.sequential import Sequential
    ...
    def test_loop_forever_rejects_non_step(self):
        from libstp.step.logic.loop import LoopForever
        with pytest.raises(TypeError, match="Step"):
            LoopForever("not a step")

    def test_loop_for_rejects_non_step(self):
        from libstp.step.logic.loop import LoopFor
        with pytest.raises(TypeError, match="Step"):
...
class TestDriveBuilderValidation:
    def test_until_wrong_type(self):
        from libstp.step.motion.drive_dsl import DriveForwardBuilder
    ...
    def test_all_drive_builders(self):
        ...
        from libstp.step.motion.drive_dsl import (
...
class TestTurnBuilderValidation:
    def test_until_wrong_type(self):
        from libstp.step.motion.turn_dsl import TurnLeftBuilder
        b = TurnLeftBuilder()
        with pytest.raises(TypeError, match="StopCondition"):
            b.until("stop")
    ...
    def test_speed_out_of_range(self):
        from libstp.step.motion.turn_dsl import TurnRightBuilder
        b = TurnRightBuilder()
        with pytest.raises(ValueError, match="speed"):
            b.speed(1.5)

    def test_all_turn_builders(self):
        from libstp.step.motion.turn_dsl import TurnLeftBuilder, TurnRightBuilder
...
class TestDslFactoryValidation:
    def test_drive_forward_builder_until_checked(self):
        ...
        from libstp.step.motion.drive_dsl import drive_forward
        b = drive_forward(cm=25)
        with pytest.raises(TypeError, match="StopCondition"):
            b.until("stop")

    def test_turn_left_builder_speed_checked(self):
        from libstp.step.motion.turn_dsl import turn_left
